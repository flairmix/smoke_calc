...
class Coridor():
    ...
    def __init__(self, coridor_system_name, level, rooms, opening_list,
                 corridor_hight, corridor_door_hight, corridor_door_width,
                 corridor_area, corridor_lenght, coef_building_type):

        self.coridor_system_name = coridor_system_name
        self.level = level
        self.rooms = rooms
    ...
    def __str__(self):
        return f"class Coridor: coridor_system_name-{self.coridor_system_name}, level:  {self.level}"
    ...
    def smoke_exhaust_coridor(self, room: Room):
        ...
        for i in range(0, len(parameter)):
            if type(parameter[i]) in (float, numpy.float64):
